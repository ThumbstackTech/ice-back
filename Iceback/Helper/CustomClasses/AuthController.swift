...
class AuthController : ObservableObject{
    ...
    func askBiometricAvailability(passcodeCompletion:@escaping(String)->(),completion: @escaping (Error?) -> Void){
        if let context{
            var failureReason : NSError?
            if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &failureReason){
                biometricType = context.biometryType
                passcodeCompletion("")
            } else if context.canEvaluatePolicy(.deviceOwnerAuthentication, error: &failureReason){
